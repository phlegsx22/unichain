...
export async function POST(req: Request) {
  try {
    ...
    if (!chainName || !chainId || !prefix || !rpcUrl || !denom || !gasPrice || !granteeAddress || !granter || !recipientAddress || !authorizedTokens || !Array.isArray(authorizedTokens)) {
      return NextResponse.json({ message: "Missing or invalid required fields" }, { status: 400 });
    }

    const database = await connectDB();
    const cosmosCollection = database.collection('nibiru_collection');
